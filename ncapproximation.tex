...
For the sake of brevity, we write \NNCO{} instead of \NNCOpoly{} in the following definition and throughout this work since we consider only a polynomial amount of nondeterminism.
...
\end{conjecture}
\begin{justification}
  \textsc{Maximum Weighted Satisfiability} is \NPO-complete \cite{ckst95} and $\NNCO=\NPO$ (TODO this needs a proof).
\end{justification}
...
TODO figure out the formal results for each of these.
...
While it is obviously important to provide efficient and highly parallel approximation algorithms for problems which are both intractible and inherently sequential, it is also desirable to provide highly parallel approximation algorithms for problems which are tractible but still inherently sequential.
