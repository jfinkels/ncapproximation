...
We adapt the clear and concise definitions of \cite{tantau07} from logspace approximability to \NC{} approximability.
...
\begin{definition}
  ...
  \item $\NNC(f(n))$ is the class of languages computable by an \NC{} circuit family augmented with $O(f(n))$ nondeterministic gates for each input length $n$ \cite{wolf94}.
    ...
    If $\mathcal{F}$ is a class of functions, then $\NNC(\mathcal{F})=\bigcup_{f\in\mathcal{F}}f(n)$.
  \end{enumerate}
\end{definition}

\NNC(\poly), also known as \GC(\poly, \NC) \cite{cc97} and $\beta\P$ \cite{kf80}, is an unusual class which may warrant some further explanation.
\NC{} has the same relationship to \NNC(\poly) as \P{} does to \NP{}.
Wolf \cite{wolf94} notes that $\NNC(\log)=\NC$ and $\NNC(\poly)=\NP$, and suggests that \NNC(\polylog) may be an interesting intermediary class, possibly incomparable with \P.
...
TODO add definition of AP reduction
...
\begin{proposition}
  Let $\mathcal{C}$ be one of the complexity classes \NCO, \NCAS, \ApxNCO, \polyApxNCO, \expApxNCO, or \NNCO.
...
\begin{conjecture}
  \textsc{Partial Maximum Weighted Satisfiability} is \NNCO-complete.
\end{conjecture}
\begin{justification}
  \textsc{Partial Maximum Weighted Satisfiability} is to \textsc{Maximum Weighted Satisfiability} as \textsc{Partial Most Valuable Vertex} is to \textsc{Most Valuable Vertex} (see \cite{tantau07}).
\end{justification}

\begin{conjecture}
  \textsc{Maximum Weighted Satisfiability} is \expApxNCO-complete.
\end{conjecture}
\begin{justification}
  \textsc{Maximum Weighted Satisfiability} is to \textsc{Maximum Bounded Weighted Satisfiability} as \textsc{Most Valuable Vertex} is to \textsc{Most Valuable Vertex Ratio} (see \cite{tantau07}).
\end{justification}
...
\end{conjecture}
\begin{justification}
  \textsc{Maximum Clique} is \polyApxPO-complete \cite[Example~2.48]{cks01}.
\end{justification}
...
\end{conjecture}
\begin{justification}
  \textsc{Minimum Set Cover} is \logApxPO-complete \cite[Example~2.48]{cks01}.
\end{justification}

In \cite{sx95}, the authors prove that Maximum Bounded Weighted Satisfiability is complete for \ApxNCO{} (there called ``\NCX'') under a less restrictive type of reduction called ``\NCAS{} reduction''.
...
\begin{conjecture}
  \textsc{Planar Maximum Satisfiability} is \NCAS-complete.
\end{conjecture}
\begin{justification}
  \textsc{Planar Maximum Satisfiability} is in \PMPSAT, which is a subset of \PTAS{} and \emph{may} be a syntactic characterization of \PTAS{} \cite{km96}.
\end{justification}

Here we need a conjecture for a \FNCAS-complete problem.
