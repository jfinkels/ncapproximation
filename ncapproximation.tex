...
\begin{proof}
  ...
  If we suppose $\NC = \P$, then it follows immediately from the definitions that any optimization problem in $\NPO$ is also in $\NNCO$.
  ...
  Hence, $S \in \NC$.
  The existence of an \NC{} algorithm for a \P-complete problem implies $\NC = \P$.
  Therefore, $\NNCO = \NPO$ if and only if $\NC = \P$.
\end{proof}
...
\begin{proof}
  ...
  If $\PO = \PO \cap \NNCO$, then there is an \NC{} algorithm for this \P-complete problem, and hence $\NC = \P$.
\end{proof}
...
\begin{proof}
  ...
  Therefore $D$ decides $L$ in \NC{}, and hence $\NC = \NNC(\poly)$.
...
\begin{proposition}\label{prop:popapxnco}
  If $\PO \cap \NNCO \subseteq \ApxNCO$ then $\NC = \P$.
\end{proposition}
\begin{proof}
...
Since the solution relation in \textsc{Double Circuit Value} requires evaluating a Boolean circuit, this problem is not in \NNCO{}, unless $\NC = \P$.
...
The reduction in the above proof is evidence that approximability is not closely related to the complexity of verification.
As discussed in \autoref{prop:popapxnco}, \textsc{Linear Programming} is in $\PO \cap \NNCO$.
This is interesting because, for the same reasons as in the proof of \autoref{prop:poppo}, \textsc{Double Circuit Value} is not in $\PO \cap \NNCO$ unless $\NC = \P$.
...
\begin{corollary}\label{cor:lpishard}
  ...
  \item \textsc{Double Circuit Value} is \emph{not} complete for the class of all maximization problems in $\PO \cap \NNCO$ under logarithmic space ??? reductions unless $\NC = \P$.
  \end{enumerate}
\end{corollary}
...
Since a constant factor \NC{} approximation algorithm for a problem complete for $\PO \cap \NNCO$ induces a constant factor \NC{} approximation algorithm for all maximization problems in $\PO \cap \NNCO$, there can be no such algorithm for \textsc{Linear Programming}, unless $\NC = \P$.
In fact, we can say something much stronger: \autoref{cor:lpishard}, \autoref{thm:hierarchy}, and the fact that ??? reductions compose imply that any highly parallel approximation for \textsc{Linear Programming} necessitates $\NC = \P$.

\begin{corollary}
  For any function $r$, \textsc{Linear Programming} does not have an $r$-approximation algorithm in \NC{} unless $\NC = \P$.
\end{corollary}
