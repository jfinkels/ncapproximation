...
We first show a maximization problem that is complete for the class of maximization problems in \PO.
...
\begin{proof}
  ...
  Furthermore, for any $x \in I_P$ and any $y \in S_C(f(x))$, we have
  \begin{equation*}
    m_P(x, g(x, y)) = m_P(x, y) = C_m(x, y) = m_C((C_E, C_m, x), y) = m_C(f(x), y).
  \end{equation*}
  Therefore, we have shown an ``S reduction'' \cite{cfs91}, one of the strictest types of approximation preserving reductions, from $P$ to \textsc{Double Circuit Value}, and hence also an ??? reduction.
\end{proof}
...
\begin{proof}
  ...
  \begin{enumerate}
  \item For each bit $x_i$, represent a 1 bit at index $i$ by $x_i = 1$ and a 0 bit by $x_i = 0$.
  \item Represent a \textsc{not} gate, $g = \lnot h$, by the inequalities $g = 1 - h$ and inequality $0 \leq g \leq 1$.
  \item Represent a \textsc{and} gate, $g = h_1 \land h_2$, by the inequalities $g \leq h_1$, $g \leq h_2$, $h_1 + h_2 - 1 \leq g$, and $0 \leq g \leq 1$.
  \item Represent an \textsc{or} gate, $g = h_1 \lor h_2$, by the inequalities $h_1 \leq g$, $h_2 \leq g$, $g \leq h_1 + h_2$, and $0 \leq g \leq 1$.
  \end{enumerate}
  Suppose $\mu_t, \dotsc, \mu_1$ are the variables corresponding to the output gates of $C_2$, numbered from least significant bit to most significant bit (that is, right-to-left).
  The components of the object function $\mathbf{c}$ are assigned to be $2^i$ where the component corresponds to the variables $\mu_i$, and 0 everywhere else.
  Define the solution transducer $g$ by $g((C_1, C_2, x), \mathbf{x})$ by \ldots.
...
As discussed in \autoref{prop:popapxnco}, \textsc{Linear Programming} is in $\PO \cap \NNCO$; this yields the following corollary.
