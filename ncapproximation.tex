...
\begin{corollary}
  $\PO \cap \NNCO$ is not closed under logarithmic space AP reductions unless $\P = \NC$.
\end{corollary}
...
If we relax the non-negativity requirement and allow a small number of equality constraints that can be violated by a small amount, then there is a fully NC approximation scheme \cite[Theorem~5.2]{tx98}.
...
The $(\gamma, \kappa)$ form of \textsc{Linear Programming} is hard to approximate \cite[Proposition~1]{efraimidis08}; the $k$-normal form reduces to \textsc{Positive Linear Programming} \cite[Theorem~2]{trevisan00}.
...
\begin{definition}[\textsc{Linear Programming with Triplets}]
  \mbox{} \\
  \begin{tabular}{r p{9.5cm}}
    \textbf{Instance:} & TODO fill me in \\
    \textbf{Solution:} & rational vectors $\mathbf{x}$ and $\mathbf{f}$ of length $n$ satisfying the inequalities. \\
    \textbf{Measure:} & $\Sigma_{i = 1}^n f_i$. \\
    \textbf{Type:} & maximization.
  \end{tabular}
\end{definition}
