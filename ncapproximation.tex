...
\end{theorem}
\begin{proof}
  We show that the minimization problem reduces to the maximization problem for \textsc{Linear Programming} by using the strong duality theorem (REFERENCE GOES HERE).
  Specifically, define the reduction $(f, g, ???)$ by $f((A, \mathbf{b}, \mathbf{c})) = (-A^\intercal, -\mathbf{c}, \mathbf{b}^\intercal)$.
  \begin{todo}
    Check that this makes sense and complete the proof.
  \end{todo}
\end{proof}
