...
\end{theorem}
\begin{proof}
  From \autoref{prop:poppo}, we know that $\PO \cap \NNCO = \PO$ implies $\NC = \P$.
...
This hierarchy theorem provides a simple proof of a result of \cite{dsst97}.
...
First, we show that \textsc{Maximum Weighted Circuit Satisfiability} is complete for the class of maximization problems in \NPO, but is not in \NNCO{} unless $\NC = \P$.
...
Using essentially the same proof as for \autoref{prop:nnconpo}, we can provide evidence that this problem is not in \NNCO.

\begin{theorem}
  If $\NC \neq \P$, then \textsc{Maximum Weighted Circuit Satisfiability} is not in \NNCO{}.
\end{theorem}

Next, we show that \textsc{Maximum Variable-Weighted Satisfiability} is complete for the class of maximization problems in \NNCO.
