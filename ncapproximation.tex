...
\newcommand{\ncasproblems}{%
  ...
  \textsc{Maximum Independent Set} \cite[Theorem 6.4.1]{dsst97}}
...
\begin{definition}
  ...
  \item $P\in\logApxNCO$ if there exists an $r$-approxiator in \FNC{} for $P$, where $r(n)=O(\lg n)$ for all $n\in\mathbb{N}$.
  ...
  \end{enumerate}
  Each of these complexity classes also has a ``polynomially bounded'' variant, denoted by the subscript $pb$, in which an optimization problem with measure $m$ has the property that there exists a polynomial $p$ such that $m(x, y) \leq p(|x|)$ for all $(x, y)\in S$.
\end{definition}
