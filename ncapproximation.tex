...
Each class also has a ``polynomially bounded'' variant, denoted by the subscript \pb, in which an optimization problem with measure $m$ has the property that there exists a polynomial $p$ such that $m(x, y) \leq p(|x|)$ for all $(x, y)\in S$.
...
\begin{definition}
  ...
  An optimization problem $Q$ is \emph{$\mathcal{C}$-hard} if for all $P\in\mathcal{C}$, we have $P\Er Q$.
...
Here we need a conjecture for a complete problem for both \polyApxNCO{} and \logApxNCO.
...
\begin{conjecture}
  \textsc{Planar MPSAT} is \NCAS-complete.
\end{conjecture}

TODO: read ``Toward a Syntactic Characterization of PTAS'' by Khanna and Motwani, in which Planar MPSAT is posed.
