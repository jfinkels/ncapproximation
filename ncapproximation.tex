...
\begin{definition}[\textsc{Linear Programming with Triplets}]
  ...
  \begin{tabular}{r p{9.5cm}}
    ...
    \item there is a constant $\gamma \in (1, \infty)$ such that $z^* \leq \gamma \mathbf{c}^\intercal \mathbf{f}$, where $z^*$ is the optimal measure of the instance.
    \end{enumerate} \\
...
Although this optimization problem is not approximable within $\frac{2 n^\epsilon}{n^\epsilon + 1}$ for any positive $\epsilon$ unless $\NC = \P$ \cite[Corollary~1]{sx02}, the corresponding \emph{evaluation problem} is $\gamma$-approximable by definition.
